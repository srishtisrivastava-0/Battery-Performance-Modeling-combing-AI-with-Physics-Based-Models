"""
Hybrid ECM + LSTM Model for Battery SOH (State of Health) Estimation
...
4. Achieve better SOH accuracy and interpretability
...
class HybridECMLSTM_SOH:
    ...
    4. Output layer: Predict State of Health (0-100%)
    """
    ...
    def extract_ecm_features(self, cycle_step=5):
        ...
        """
        print("Step 1: Extracting ECM parameters for SOH...")
    ...
    def prepare_hybrid_features(self):
        """
        Combine raw measurements with ECM parameters for SOH
    ...
    def build_model(self, input_shape):
        """
        Build hybrid LSTM model for SOH
    ...
    def evaluate(self, X, y):
        """
        Evaluate SOH model performance
        ...
        print("\n" + "="*60)
        print("SOH Estimation Performance:")
        print("="*60)
    ...
    def plot_soh_results(self, y_true, y_pred, save_path=None):
        """
        Plot SOH prediction results
        ...
        axes[0, 0].set_title('Hybrid ECM-LSTM: SOH Prediction vs Actual', fontsize=14, fontweight='bold')
        axes[0, 0].grid(True, alpha=0.3)
        
        # Time series
        axes[0, 1].plot(y_true, 'b-o', label='True SOH', markersize=6, linewidth=2)
        axes[0, 1].plot(y_pred, 'r--s', label='Predicted SOH', markersize=6, linewidth=2)
        axes[0, 1].set_xlabel('Cycle Index', fontsize=12)
        axes[0, 1].set_ylabel('SOH (%)', fontsize=12)
        axes[0, 1].set_title('SOH Degradation Over Time', fontsize=14, fontweight='bold')
        ...
        axes[1, 0].set_title('SOH Error Distribution', fontsize=14, fontweight='bold')
        ...
        # Error over time
        axes[1, 1].plot(error, 'g-o', markersize=6, linewidth=2)
        axes[1, 1].set_xlabel('Cycle Index', fontsize=12)
        axes[1, 1].set_ylabel('Prediction Error (%)', fontsize=12)
        axes[1, 1].set_title('SOH Error Over Time', fontsize=14, fontweight='bold')
    ...
    def compare_with_baseline(self, X, y):
        ...
        """
        print("\n" + "="*60)
        print("Comparing Hybrid vs Baseline LSTM for SOH")
        print("="*60)
        ...
        print("-"*60)
        print(f"{'Baseline LSTM':<20} {rmse_baseline:<12.4f} {mae_baseline:<12.4f} {r2_baseline:<12.6f}")
        print(f"{'Hybrid ECM-LSTM':<20} {metrics_hybrid['RMSE']:<12.4f} {metrics_hybrid['MAE']:<12.4f} {metrics_hybrid['R2']:<12.6f}")
        print("-"*60)
...
def run_hybrid_soh_analysis(battery_id='B0005'):
    """
    Complete hybrid ECM-LSTM SOH analysis workflow
    
    Args:
        battery_id: Battery to analyze
    """
    print("="*60)
    print(f"Hybrid ECM-LSTM SOH Analysis - Battery {battery_id}")
    print("="*60)
    
    # Initialize hybrid model
    hybrid = HybridECMLSTM_SOH(battery_id=battery_id, sequence_length=50)
    ...
    # Train model
    history = hybrid.train(X, y, epochs=150, batch_size=16)
    
    # Evaluate
    metrics, y_pred = hybrid.evaluate(X, y)
    
    # Plot results
    hybrid.plot_soh_results(y, y_pred, save_path=f'hybrid/results/{battery_id}_hybrid_soh_results.png')
    
    # Compare with baseline
    comparison = hybrid.compare_with_baseline(X, y)
    
    print("\n✅ Hybrid SOH analysis complete!")
    print(f"Results saved to: hybrid/results/{battery_id}_hybrid_soh_results.png")
    
    return hybrid, metrics, comparison


if __name__ == "__main__":
    # Run hybrid SOH analysis
    hybrid, metrics, comparison = run_hybrid_soh_analysis('B0005')
